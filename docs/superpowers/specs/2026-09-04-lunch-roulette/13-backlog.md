...
| B11 | 식당 수집 전체 마감 시한 | 17 | 호출당 타임아웃은 8초(http.ts)지만 전체 마감 시한이 없다. 최악 순차 깊이 24회라 카카오가 느리면 maxDuration 30초를 넘겨 플랫폼 504가 갈 수 있다. 실측 1.1초라 당장은 문제 없음. B3 재수집과 함께 본다. |
| B12 | RPC 식당 삽입 set 기반 전환 | 17 | create_place_with_restaurants가 식당당 insert 2회 루프라 200개면 400문. 상한을 더 올릴 때 jsonb_to_recordset 기반 일괄 삽입으로 바꾼다. |
...
- (구 B10, 번호는 내 정보 확장에 재사용) 식당 수집 45개 상한 넘기기 → 스펙 17로 설계 확정, 구현·배포 완료 (2026-09-07).
