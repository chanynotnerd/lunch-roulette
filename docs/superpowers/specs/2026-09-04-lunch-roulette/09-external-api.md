...
- 카카오 할당량: 키워드 검색·주소 검색 각각 앱당 일 100,000회, 무료 API 전체 월 3,000,000회, 결제 수단 등록 없음(2026-09-07 콘솔 확인). 넘으면 429가 오고 앱은 EXTERNAL_API_UNAVAILABLE을 보여 준다. 과금은 발생하지 않는다.
- 인증은 `Authorization: KakaoAK <REST API 키>` 헤더. 결제 등록이 필요 없다.
